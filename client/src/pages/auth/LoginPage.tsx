      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl border border-slate-800 bg-[#0c1326]/90 p-8 backdrop-blur-2xl shadow-2xl shadow-black/40">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Access intelligence analytics, data visualization & forecasting
          </p>
        </div>

        {error && (
          <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-3.5 text-xs text-rose-300 animate-in fade-in space-y-2">
            ...
            {error.includes('Register') && (
              <div className="pt-1 border-t border-rose-500/20 flex justify-end">
                <Link
                  to="/register"
                  className="inline-flex items-center space-x-1 font-bold text-cyan-400 hover:text-cyan-300 hover:underline text-xs"
                >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            <span>{isSubmitting ? 'Authenticating...' : t('signIn', 'Sign In')}</span>
